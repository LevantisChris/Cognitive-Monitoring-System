...
package com.levantis.logboard.keyboard;
...
import com.levantis.logboard.keyboard.internal.BogusMoveEventDetector;
import com.levantis.logboard.keyboard.internal.DrawingProxy;
import com.levantis.logboard.keyboard.internal.PointerTrackerQueue;
import com.levantis.logboard.keyboard.internal.TimerProxy;
import com.levantis.logboard.latin.common.Constants;
import com.levantis.logboard.latin.common.CoordinateUtils;
import com.levantis.logboard.latin.define.DebugFlags;
import com.levantis.logboard.latin.settings.Settings;
...
public final class PointerTracker implements PointerTrackerQueue.Element {
    ...
    private Key mLastLoggedKey = null;
    ...
    /* (PauseCtC) Tracks in which char the timer has started for the first pair, so we can know where to end it */
    private Key activeStartCharacterONE_pauseCtC = null;
    /* (PauseCtC) Tracks in which char the timer has started for the second pair, so we can know ehere to end it */
    private Key activeStartCharacterTWO_pauseCtC = null;
    /* (IKI) Tracks in which char the timer has started for the IKI, so we can know where to end it */
    private Key activeStartCharacter_IKI = null;
    public void processMotionEvent(final MotionEvent me, final KeyDetector keyDetector) {
        final int action = me.getActionMasked();
        final long eventTime = me.getEventTime();

        try {
            Key detectedKey = mKeyDetector.detectHitKey((int)me.getX(), (int)me.getY());
            if (action == ACTION_MOVE) {
                Log.d("PointerTracker inside", "processMotionEvent ACTION_MOVE: " + (int)me.getX() + " " + (int)me.getY() + " " + action + " " + eventTime);
            ...
            switch (action) {
                ...
                case ACTION_DOWN:
                    Log.d("PointerTracker inside", "processMotionEvent ACTION_DOWN: " + x + " " + y + " " + action + " " + eventTime + ", detectedKey: " + detectedKey);

                    /* Handle PauseCtC timer for Down event  */
                    if (detectedKey != null && mKeyboard != null && mKeyboard.mInputSession != null
                            && mKeyboard.mInputSession.activeTypingSession != null
                            && (isEnglishLetter(detectedKey.getCode()) || isGreekLetter(detectedKey.getCode()))) {
                        handlePauseCtCTimer_DownEvent(detectedKey);
                    }

                    /* Handle IKI timer for Down event  */
                    if(detectedKey != null && mKeyboard != null && mKeyboard.mInputSession != null
                            && mKeyboard.mInputSession.activeTypingSession != null
                            && !isSpecialKey(detectedKey.getCode())
                    ) {
                        handleIIkiTimer_DownEvent(detectedKey);
                    }
                ...
                case MotionEvent.ACTION_POINTER_UP:
                    Log.d("PointerTracker inside", "processMotionEvent ACTION_UP: " + x + " " + y + " " + action + " " + eventTime + ", detectedKey: " + detectedKey);

                    /* Handle PauseCtC timer for Up event  */
                    if (detectedKey != null && mKeyboard != null && mKeyboard.mInputSession != null
                            && mKeyboard.mInputSession.activeTypingSession != null
                            && (isEnglishLetter(detectedKey.getCode()) || isGreekLetter(detectedKey.getCode()))) {
                        handlePauseCtCTimer_UpEvent(detectedKey);
                    }

                    /* Handle IKI timer for Up event  */
                    if(detectedKey != null && mKeyboard != null && mKeyboard.mInputSession != null
                            && mKeyboard.mInputSession.activeTypingSession != null
                            && !isSpecialKey(detectedKey.getCode())
                    ) {
                        handleIIkiTimer_UpEvent(detectedKey);
                    }

                    onUpEvent(x, y, eventTime);
                    break;
                case MotionEvent.ACTION_CANCEL:
                    Log.d("PointerTracker inside", "processMotionEvent ACTION_CANCEL: " + x + " " + y + " " + action + " " + eventTime);
                    onCancelEvent(x, y, eventTime);
                    break;
            }
        } catch (Exception e) {
            Log.e("PointerTracker", "Exception in processMotionEvent: " + e.getMessage());
            throw e;
        }
    }

    private void handlePauseCtCTimer_DownEvent(Key detectedKey) {
        // If we have a previous character, start a new pause measurement
        if (mKeyboard.mInputSession.activeTypingSession.activePauseCtCList.get(0) == null) {
            Log.d("PointerTracker inside", "Start PauseCtC timer for character START_KEY ONE: " + detectedKey);
            startPauseCtCTimer(0);
            activeStartCharacterONE_pauseCtC = detectedKey;
        } else if(mKeyboard.mInputSession.activeTypingSession.activePauseCtCList.get(1) == null) {
            Log.d("PointerTracker inside", "Start PauseCtC timer for character START_KEY TWO: " + detectedKey);
            startPauseCtCTimer(1);
            activeStartCharacterTWO_pauseCtC = detectedKey;
        }
    }

    private void handlePauseCtCTimer_UpEvent(Key detectedKey) {
        // End the current pause measurement if one exists
        if (mKeyboard.mInputSession.activeTypingSession.activePauseCtCList.get(0) != null && activeStartCharacterONE_pauseCtC != detectedKey) {
            Log.d("PointerTracker inside", "End PauseCtC timer for character ONE: " + activeStartCharacterONE_pauseCtC + " -> " + detectedKey);
            endPauseCtCTimer(mKeyboard.mInputSession.activeTypingSession.activePauseCtCList.get(0), 0);
            activeStartCharacterONE_pauseCtC = null;
        } else if(mKeyboard.mInputSession.activeTypingSession.activePauseCtCList.get(1) != null && activeStartCharacterTWO_pauseCtC != detectedKey) {
            Log.d("PointerTracker inside", "End PauseCtC timer for character TWO: " + activeStartCharacterTWO_pauseCtC + " -> " + detectedKey);
            endPauseCtCTimer(mKeyboard.mInputSession.activeTypingSession.activePauseCtCList.get(1), 1);
            activeStartCharacterTWO_pauseCtC = null;
        }
    }

    private void handleIIkiTimer_DownEvent(Key detectedKey) {
        TypingSession activeTypingSession = mKeyboard.mInputSession.activeTypingSession;
        if(activeTypingSession.activeIKI != null && activeStartCharacter_IKI != detectedKey) {
            activeTypingSession.activeIKI.endIKI();
            activeTypingSession.activeIKI = null;
            activeStartCharacter_IKI = null;
        }
    }

    private void handleIIkiTimer_UpEvent(Key detectedKey) {
        TypingSession activeTypingSession = mKeyboard.mInputSession.activeTypingSession;
        if(activeTypingSession.activeIKI == null) {
            IKI ikiMetric = new IKI();
            activeTypingSession.addIki(ikiMetric);
            activeTypingSession.activeIKI = ikiMetric;
            ikiMetric.startIKI();
            //
            activeStartCharacter_IKI = detectedKey;
        }
    }
    ...
    public void onLongPressed() {
        ...
        //TODO: Here capture SPACE press (but long)
    ...
    /* Here you can not catch secondary keys, like ? for example */
    private void detectAndSendKey(final Key key, final int x, final int y) {
        if (key == null) return;

        final int code = key.getCode();

        /* Find in which layout the keyboard is, Greek or English.
        *  That's because different layouts have different special
        *  characters
        * TODO: Check if subtype en_US:qwerty and subtype en_GB:qwerty have the same basic layouts
        * */
        if(mKeyboard.mId.mSubtype.toString().equals("subtype en_US:qwerty") || mKeyboard.mId.mSubtype.toString().equals("subtype en_GB:qwerty")) {
            detectBasedOnEnglishLayout(key);
        } else if(mKeyboard.mId.mSubtype.toString().equals("subtype el:greek")) {
            detectBasedOnGreekLayout(key);
        }
    ...
    /**
     * Detect key code for a English keyboard layout
     * */
    private void detectBasedOnEnglishLayout(Key key) {
        if(isEnglishLetter(key.getCode())) {
            Log.d("KEY", "ENGLISH LETTER");
            if (mKeyboard.mInputSession != null
                    && mKeyboard.mInputSession.activeTypingSession != null
            ) {
                mKeyboard.mInputSession.activeTypingSession.characterTyped(); // increment the characters counter by one
                endPauseWtWTime();
            }
        } else if(key.getCode() == 32) { // Space pressed
            Log.d("KEY", "SPACE");
            if (mKeyboard.mInputSession != null
                    && mKeyboard.mInputSession.activeTypingSession != null
            ) {
                mKeyboard.mInputSession.activeTypingSession.wordTyped(); // increment the words counter by one
                startPauseWtWTimer();
            }
    ...
    /**
     * Detect key code for a Greek keyboard layout
     * */
    private void detectBasedOnGreekLayout(Key key) {

        if(isGreekLetter(key.getCode())) {
            Log.d("KEY", "GREEK LETTER");
            if (mKeyboard.mInputSession != null
                    && mKeyboard.mInputSession.activeTypingSession != null
            ) {
                mKeyboard.mInputSession.activeTypingSession.characterTyped(); // increment the characters counter by one
    ...
    /**
     * I need a way to detect of the key pressed is a letter, current implementation
     *  did not differentiate between letters and special characters, so this is a
     *  fast way to do it
     * */
    private boolean isEnglishLetter(int code) {
        return (code >= 'a' && code <= 'z') || (code >= 'A' && code <= 'Z');
    }

    private boolean isGreekLetter(int code) {
        // Greek letters Unicode ranges
        return (code >= 0x0391 && code <= 0x03A9) || // Greek uppercase
                (code >= 0x03B1 && code <= 0x03C9);    // Greek lowercase
    }

    /* Exclude all the special characters:
    *   Space --> 32
    *   Change language --> -10
    *    Enter --> 10
    *    Delete/remove --> -5
    *    Go to special symbols (=\<) --> -12
    *    Go to special chars (ABC and ΑΒΓ and ?123) --> -3
    *  Shift --> -1
    * */
    private boolean isSpecialKey(int code) {
        return code == 32 || code == -10 || code == 10 || code == -5 || code == -12 || code == -3 || code == -1;
    }

    /* In the current active typing session start and add a pause (word to word)
     *
     * We take the current active typing session in the current input session
     * running. Then we crate a new pause instance and set it as active for
     * the current typing session. Finally we add it in the list of active pauses in the
     * typing session and also we start the timer (pauseDuration_start) for that pause.
     *
     * Also to start a pause timer the user need to have typed at least two words.*/
    private void startPauseWtWTimer() {
        TypingSession activeTypingSession = mKeyboard.mInputSession.activeTypingSession;
        //
        if (activeTypingSession.activePauseWtW == null && activeTypingSession.wordsTyped >= 2) {
            PauseWtW pauseWtW = new PauseWtW(activeTypingSession);
            activeTypingSession.addPauseWtW(pauseWtW); // but in the list of active pauses
            activeTypingSession.activePauseWtW = pauseWtW;
            pauseWtW.startPauseDuration(); // the timer for the pauseWtW starts
        }
    }

    /* We take the active pause a the current typing session and end it. But, if
     *  active pause is null that means it never started, that an error. */
    private void endPauseWtWTime() {
        TypingSession activeTypingSession = mKeyboard.mInputSession.activeTypingSession;
        //
        PauseWtW activePauseWtW = activeTypingSession.activePauseWtW;
        if (activePauseWtW != null) {
            activePauseWtW.endPauseDuration(); // the timer for the pause ends
            activeTypingSession.activePauseWtW = null;
        }
    }

    /* Same as in startPauseWtWTimer but for character to character */
    private void startPauseCtCTimer(int position) {
        TypingSession activeTypingSession = mKeyboard.mInputSession.activeTypingSession;
        // && activeTypingSession.charactersTyped >= 10
        if(activeTypingSession.activePauseCtCList.get(position) == null) {
            Log.d("PointerTracker", "startPauseCtCTimer for position: " + position + " in active pause list.");
            PauseCtC activePauseCtCToStart = new PauseCtC(activeTypingSession); // new pause ctc instance
            activeTypingSession.addPauseCtC(activePauseCtCToStart);
            activeTypingSession.activePauseCtCList.set(position, activePauseCtCToStart); // add it in the list of active pauses in the position it is
            activePauseCtCToStart.startPauseTimer(); // the timer for the pauseCtC starts
        }
    }

    /* Same as in endPauseWtWTimer but for character to character */
    private void endPauseCtCTimer(PauseCtC activePauseCtCToEnd, int position) {
        Log.d("PointerTracker", "endPauseCtCTimer");
        TypingSession activeTypingSession = mKeyboard.mInputSession.activeTypingSession;
        //
        //PauseCtC activePauseCtC = activeTypingSession.activePauseCtC;
        if(activePauseCtCToEnd != null) {
            activePauseCtCToEnd.endPauseTimer();
            activeTypingSession.activePauseCtCList.set(position, null);
        }
    }
