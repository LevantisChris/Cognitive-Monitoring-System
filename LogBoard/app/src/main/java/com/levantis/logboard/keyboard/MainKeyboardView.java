...
import com.levantis.logboard.keyboard.internal.DrawingPreviewPlacerView;
import com.levantis.logboard.keyboard.internal.DrawingProxy;
import com.levantis.logboard.keyboard.internal.KeyDrawParams;
import com.levantis.logboard.keyboard.internal.KeyPreviewChoreographer;
import com.levantis.logboard.keyboard.internal.KeyPreviewDrawParams;
import com.levantis.logboard.keyboard.internal.KeyPreviewView;
import com.levantis.logboard.keyboard.internal.MoreKeySpec;
import com.levantis.logboard.keyboard.internal.NonDistinctMultitouchHelper;
import com.levantis.logboard.keyboard.internal.TimerHandler;
import com.levantis.logboard.latin.RichInputMethodManager;
import com.levantis.logboard.latin.Subtype;
import com.levantis.logboard.latin.common.Constants;
import com.levantis.logboard.latin.common.CoordinateUtils;
import com.levantis.logboard.latin.utils.LanguageOnSpacebarUtils;
import com.levantis.logboard.latin.utils.LocaleResourceUtils;
import com.levantis.logboard.latin.utils.TypefaceUtils;
...
public final class MainKeyboardView extends KeyboardView implements MoreKeysPanel.Controller, DrawingProxy {
    ...
    public boolean processMotionEvent(final MotionEvent event) {
        ...
        processPressureEvent(event, index);
    ...
    /**
     * This function will detect the amount of pressure applied to the screen in a character.
     * But this will NOT be done using the build in function getPressure() from the MotionEvent class.
     * This is because not all devices support this function natively, to bypass this I will detect how many events (0, 1 and 2)
     * have been generated. More precisely how many times the function processMotionEvent has been called for a click.
     *
     * If the user presses a key for a long time, more events will be generated, and the pressure will be higher.
     * If the user presses a key in a normal way, 3 to 8 events will be generated.
     * if the keys presses the key in a fast way, 1 to 2 events will be generated.
     * and so on...
     *
     * The pressure detection will be calculated only for letters, and numbers.
     *
     * NOTE: In some cases getPressure and getSize will return the same value
     *  */
    private  int  eventCount = 0;
    private void processPressureEvent(MotionEvent event, int index) {
        float x = event.getX(index);
        float y = event.getY(index);
        Key key = mKeyDetector.detectHitKey((int) x, (int) y);

        // Skip if no key detected
        if (key == null) {
            return;
        }

        // Get current typing session
        TypingSession activeSession = mKeyDetector.getKeyboard().mInputSession.activeTypingSession;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                // Only count pressure for letters
                if (isEnglishLetter(key.getCode()) || isGreekLetter(key.getCode())) {
                    eventCount++;
                    activeSession.addPressureCounter(1); // Add incrementally
                }
                break;
            case MotionEvent.ACTION_UP:
                // If showing more keys panel when lifting finger, remove the pressure count
                if (isShowingMoreKeysPanel() && eventCount > 0) {
                    activeSession.removePressureCounter(eventCount);
                }
                // Reset counter
                eventCount = 0;
                break;
